...
int digenv(int argc, char* argv[])
{
    ...
    /* Set jump point instead of using exit when something goes wrong as
     * program will not be run in a separate process */
    int val = setjmp(jmp);
    if (val)
        return 5;
...
*/                kill(pid[i], 9);
    ...
    /* Use longjmp instead of exit as we don't have our own process anymore */
    longjmp(jmp,1);
}
